(function(){var index=angular.module("index",["ngResource","ngRoute","ngSanitize","data","resources","helper","login","comments","googlechart","brew-o-module.controller","notification","abm","gt.abm","admin","ui.bootstrap","alerts","env"]);index.config(["$routeProvider",function($routeProvider){$routeProvider.when("/recipe",{templateUrl:"partial/recipe-list.html",controller:"RecipeListCtrl"}).when("/collaborated",{templateUrl:"partial/recipe-collaborated.html",controller:"RecipeCollaboratedCtrl"}).when("/favorites",
{templateUrl:"partial/recipe-favorite.html",controller:"RecipeFavoriteCtrl"}).when("/public",{templateUrl:"partial/recipe-public.html",controller:"RecipePublicCtrl"}).when("/home/:userId",{templateUrl:"partial/user/home.html",controller:"HomeCtrl"}).when("/recipe/edit/:recipeId",{templateUrl:"partial/recipe-detail.html",controller:"RecipeDetailCtrl"}).when("/recipe/clone/:recipeId",{templateUrl:"partial/recipe-detail.html",controller:"RecipeDetailCtrl"}).when("/recipe/new",{templateUrl:"partial/recipe-detail.html",
controller:"RecipeDetailCtrl"}).when("/settings",{templateUrl:"partial/user/user-settings.html",controller:"UserSettingsCtrl"}).when("/notification",{templateUrl:"partial/user/user-notification.html",controller:"NotificationsCtrl"}).when("/data/:entity",{templateUrl:"partial/data/abm.html",controller:"AbmCtrl"}).when("/admin/:entity",{templateUrl:"partial/admin/admin.html",controller:"AdminCtrl"}).otherwise({redirectTo:"/recipe"})}]);index.controller("HomeCtrl",function($scope,$rootScope,User,Recipe,
$routeParams){$scope.$watch("user",function(){$scope.viewUser=User.get({id:$routeParams.userId},function(){$rootScope.breadcrumbs=[{link:"#",title:"Home"},{link:"#",title:$scope.viewUser.name}]});$scope.recipes=Recipe.findByUser({id:$routeParams.userId})});$scope.addFavorites=function(recipe){User.addToFavorites(recipe,function(user){$rootScope.user.favorites=user.favorites})};$scope.removeFavorites=function(recipe){User.removeFromFavorites(recipe,function(user){$rootScope.user.favorites=user.favorites})}});
var notification=angular.module("notification",[]);notification.factory("notificationData",function(){return{listener:null,reset:function(){if(this.listener)this.listener()}}});notification.controller("NotificationsCtrl",function($scope,Notification,$rootScope,notificationData){notificationData.reset();$scope.updateCount=function(notifications){$scope.countUnread=0;$scope.countNew=0;angular.forEach(notifications,function(not){if(not.status=="new")$scope.countNew++;else if(not.status=="unread")$scope.countUnread++})};
$scope.$watch("user",function(){$scope.notifications=Notification.query($scope.updateCount)});$rootScope.breadcrumbs=[{link:"#",title:"Home"},{link:"#",title:"Notificaciones"}];$scope.markAsRead=function(notification){if(notification.status!="read"){notification.status="read";notification.$save();$scope.updateCount($scope.notifications)}};$scope.statusClass=function(notification){if(notification.status=="unread")return"gt-notification-unread";else if(notification.status=="new")return"gt-notification-new";
return""}});index.controller("UserSettingsCtrl",function($scope,User,$rootScope){$scope.disconnectUser=function(){var revokeUrl="https://accounts.google.com/o/oauth2/revoke?token="+gapi.auth.getToken().access_token;$.ajax({type:"GET",url:revokeUrl,async:false,contentType:"application/json",dataType:"jsonp",success:function(nullResponse){$rootScope.user=undefined;$scope.$apply()},error:function(e){}})};$rootScope.breadcrumbs=[{link:"#",title:"Home"},{link:"#",title:"Configuracion"}];$scope.notifications=
[];$scope.save=function(){User.updateSettings($scope.user,function(){$scope.notifications.push({type:"success",title:"Configuracion guardada!",text:"Tus cambios han sido guardados con exito!"})})}});index.controller("MainController",function($scope,$rootScope){$rootScope.breadcrumbs=[];$scope.login=function(){var button=$($($("#signinButton").children()[0]).children()[0]);button.click()}});var alerts=angular.module("alerts",[]);alerts.factory("alertFactory",function($rootScope){var alerts=[];return{create:function(type,
text,title){alerts=[];var a={type:type,text:text,title:title};alerts.push(a);setTimeout(function(){util.Arrays.remove(alerts,a);$rootScope.$apply()},5E3)},getAlerts:function(){return alerts}}});index.run(function($rootScope,version,$filter,$location,BrewCalc,env,color,alertFactory){$rootScope.BrewCalc=BrewCalc;$rootScope.version=version;$rootScope.env=env;$rootScope.color=color;$rootScope.getAlerts=function(){return alertFactory.getAlerts()};$rootScope.encodeName=function(name){return encodeURIComponent(name)};
$rootScope.sharedUrl=function(_id){return"http://"+$location.host()+":"+$location.port()+"/share.html#/"+_id};$rootScope.formatDate=function(date){return util.formatDate(date,$filter("date"))}})})();(function(){var helper=angular.module("helper",[]);helper.directive("showTags",function($compile,TagColor){return{restrict:"EA",replace:true,scope:{tags:"&",removable:"@",itemClick:"&"},template:"<button style='margin:2px' ng-click='removeTag($index)' ng-repeat='tag in tags() track by $index' type='button' class='btn btn-xs' ng-class='color(tag)'>"+"{{tag}}"+"<span ng-show='removable' class='glyphicon glyphicon-remove'></span>"+"</button>",controller:function($scope){$scope.color=TagColor;$scope.removeTag=
...
colorsStyles[colorPos++];colorPos%=colorsStyles.length;colorsTags[tag]=ret;return ret}}});helper.factory("BrewCalc",function(BrewHelper){return{evapTotal:function(BOIL_TIME,EvapPerHour){var hours=Math.floor(BOIL_TIME/60);var rest=BOIL_TIME%60/60;var percentageEvap=1-rest*EvapPerHour/100;for(var i=0;i<hours;i++)percentageEvap*=1-EvapPerHour/100;return 1-percentageEvap},calculateBoilSize:function(BATCH_SIZE,TrubChillerLosses,BOIL_TIME,PercentEvap,TopUpWater){var ltsAfterBoil=BATCH_SIZE/0.94+TrubChillerLosses;
var percentageEvap=this.evapTotal(BOIL_TIME,PercentEvap);var tuw=TopUpWater?TopUpWater:0;return ltsAfterBoil/(1-percentageEvap)+tuw},initialMashVolume:function(StrikeWater,totalAmount){return StrikeWater+totalAmount},actualMashVolume:function($index,initVol,steps){var liters=initVol;for(var i=0;i<=$index;i++){var it=steps[i];if(it.infuse)liters+=it.INFUSE_AMOUNT}return liters},estimateLiters:function($index,BATCH_SIZE,stages){var liters=BATCH_SIZE;for(var i=0;i<$index;i++){var it=stages[i];if(it.transferring)liters-=
it.losses}return liters},bottledLiters:function(volumeByCarbonatationType,bottles){var liters=0;volumeByCarbonatationType.sugar=0;volumeByCarbonatationType.must=0;volumeByCarbonatationType.co2=0;angular.forEach(bottles,function(bottle){liters+=bottle.size*bottle.amount;volumeByCarbonatationType[bottle.carbonatationType]+=bottle.size*bottle.amount});return liters},fixEmptyValues:function(recipe){recipe.TrubChillerLosses=recipe.TrubChillerLosses||0;recipe.mashTemp=recipe.mashTemp||66;recipe.GrainTemp=
recipe.GrainTemp||25;recipe.SpargeTempDesired=recipe.SpargeTempDesired||75;recipe.SpargeDeadSpace=recipe.SpargeDeadSpace||0;recipe.lossMashTemp=recipe.lossMashTemp||0;recipe.PercentEvap=recipe.PercentEvap||10;if(!recipe.WatertoGrainRatio){recipe.WatertoGrainRatio=3;recipe.StrikeWater=BrewHelper.round(recipe.WatertoGrainRatio*recipe.totalAmount,10)}}}});helper.factory("BrewHelper",function(){return{toLbs:function(kg){return kg/0.45359},toOz:function(kg){return kg*1E3*0.035274},toGal:function(liters){return liters*
0.264172052637296},toPpg:function(potential){return potential*1E3-1E3},toPotential:function(ppg){return this.round((ppg+1E3)/1E3,1E3)},round:function(value,zeros){return Math.round(value*zeros)/zeros},pad:function(value,zeros){return util.pad(value,zeros)},calculateU:function(gravity,time){var g=this.toPpg(gravity);var m=30;var M=120;for(var i=0;i<U_gravity.length;i++)if(g<U_gravity[i]){M=U_gravity[i];break}else m=U_gravity[i];var diff=M-m;var p=(g-m)/diff;if(p==Infinity||isNaN(p))p=0;var valm;var valM;
if(U[time.toString()]){valm=U[time.toString()][m.toString()];valM=U[time.toString()][M.toString()]}else{valm=0;valM=0}var valDiff=valM-valm;var valP=valDiff*p;return valm+valP},convertColor:function(srm){if(srm>40)return"black";else if(srm<1)return"white";else return SRM[Math.round(srm)]}}});var SRM=["#FFFFFF","FFE699","#FFD878","#FFCA5A","#FFBF42","#FBB123","#F8A600","#F39C00","#EA8F00","#E58500","#DE7C00","#D77200","#CF6900","#CB6200","#C35900","#BB5100","#B54C00","#B04500","#A63E00","#A13700",
"#9B3200","#952D00","#8E2900","#882300","#821E00","#7B1A00","#771900","#701400","#6A0E00","#660D00","#5E0B00","#5A0A02","#600903","#520907","#4C0505","#470606","#440607","#3F0708","#3B0607","#3A070B","#36080A"];var U_time=[0,5,10,15,20,25,30,35,40,45,50,55,60,70,80,90,100,110,120];var U_gravity=[30,40,50,60,70,80,90,100,110,120];var U={0:{30:0,40:0,50:0,60:0,70:0,80:0,90:0,100:0,110:0,120:0},5:{30:0.055,40:0.05,50:0.046,60:0.042,70:0.038,80:0.035,90:0.032,100:0.029,110:0.027,120:0.025},10:{30:0.1,
40:0.091,50:0.084,60:0.076,70:0.07,80:0.064,90:0.058,100:0.053,110:0.049,120:0.045},15:{30:0.137,40:0.125,50:0.114,60:0.105,70:0.096,80:0.087,90:0.08,100:0.073,110:0.067,120:0.061},20:{30:0.167,40:0.153,50:0.14,60:0.128,70:0.117,80:0.107,90:0.098,100:0.089,110:0.081,120:0.074},25:{30:0.192,40:0.175,50:0.16,60:0.147,70:0.134,80:0.122,90:0.112,100:0.102,110:0.094,120:0.085},30:{30:0.212,40:0.194,50:0.177,60:0.162,70:0.148,80:0.135,90:0.124,100:0.113,110:0.103,120:0.094},35:{30:0.229,40:0.209,50:0.191,
60:0.175,70:0.16,80:0.146,90:0.133,100:0.122,110:0.111,120:0.102},40:{30:0.242,40:0.221,50:0.202,60:0.185,70:0.169,80:0.155,90:0.141,100:0.129,110:0.118,120:0.108},45:{30:0.253,40:0.232,50:0.212,60:0.194,70:0.177,80:0.162,90:0.148,100:0.135,110:0.123,120:0.113},50:{30:0.263,40:0.24,50:0.219,60:0.2,70:0.183,80:0.168,90:0.153,100:0.14,110:0.128,120:0.117},55:{30:0.27,40:0.247,50:0.226,60:0.206,70:0.188,80:0.172,90:0.157,100:0.144,110:0.132,120:0.12},60:{30:0.276,40:0.252,50:0.231,60:0.211,70:0.193,
80:0.176,90:0.161,100:0.147,110:0.135,120:0.123},70:{30:0.285,40:0.261,50:0.238,60:0.218,70:0.199,80:0.182,90:0.166,100:0.152,110:0.139,120:0.127},80:{30:0.291,40:0.266,50:0.243,60:0.222,70:0.203,80:0.186,90:0.17,100:0.155,110:0.142,120:0.13},90:{30:0.295,40:0.27,50:0.247,60:0.226,70:0.206,80:0.188,90:0.172,100:0.157,110:0.144,120:0.132},100:{30:0.298,40:0.272,50:0.249,60:0.228,70:0.208,80:0.19,90:0.174,100:0.159,110:0.145,120:0.133},110:{30:0.3,40:0.274,50:0.251,60:0.229,70:0.209,80:0.191,90:0.175,
100:0.16,110:0.146,120:0.134},120:{30:0.301,40:0.275,50:0.252,60:0.23,70:0.21,80:0.192,90:0.176,100:0.161,110:0.147,120:0.134}}})();(function(){var module=angular.module("brew-o-module.controller",[]);module.controller("RecipeMashCtrl",function($scope,BrewCalc,BrewHelper){$scope.styleTitle=function(onFocus){if(onFocus)return{background:"white","border-color":"#ccc"};else return{background:"white","border-color":"white",cursor:"pointer"}};$scope.moment=function($index){var time=0;for(var i=0;i<$index;i++)time+=$scope.recipe.MASH.MASH_STEPS.MASH_STEP[i].STEP_TIME;return time};$scope.totalTime=function(){var time=0;angular.forEach($scope.recipe.MASH.MASH_STEPS.MASH_STEP,
...
return}if(user_id==$scope.recipe.owner._id){alertFactory.create("warning","No puede agregar al due\u00f1o de la receta como colaborador","Error al agregar usuario");return}var user=util.Arrays.filter($scope.users,filter)[0];$scope.recipe.collaborators.push(user)}});module.controller("RecipeTabCtrl",function($scope){$scope.sortTabs=["main","mash","boil","fermentation","bottling","log","collaborators"];$scope.tabs={main:{title:"Receta",template:"detail-main"},mash:{title:"Macerado",template:"mash"},
boil:{title:"Hervido",template:"boil"},fermentation:{title:"Fermentacion",template:"fermentation"},bottling:{title:"Embotellado",template:"bottling"},log:{title:"Bitacora",template:"log"},collaborators:{title:"Colaboradores",template:"collaborators"}};$scope.selectedTab="main";$scope.getActiveClass=function(tab){return $scope.selectedTab===tab?"active":""};$scope.changeTab=function(tab){$scope.selectedTab=tab;$scope.$parent.notifications=[]}})})();(function(){var index=angular.module("index");index.filter("limitText",function(){return function(value,limit){if(value.length>limit)return value.substring(0,limit)+"...";else return value}});index.filter("filterFavorites",function(){return function(list,favorites){var ret=[];if(list&&list.length!=0&&favorites&&favorites.length!=0)angular.forEach(list,function(recipe){if(favorites.indexOf(recipe._id)!=-1)ret.push(recipe)});return ret}});index.controller("RecipeCollaboratedCtrl",function($scope,$rootScope,
...
Tag){$scope.sort=sortData("publishDate","-");$scope.styles=Style.query();$scope.tags=Tag.query();$scope.showAd=false;$scope.filterData={};$scope.filterData["STYLE.NAME"]={comparator:"equal",ignoreCase:false};$scope.filterData["NAME"]={comparator:"like",ignoreCase:true};$scope.filterData["tags"]={comparator:"searchIn",type:"list",ignoreCase:true};angular.forEach($scope.filterData,function(f,key){if($location.$$search[key]){$scope.showAd=true;if(f.type=="list")$scope.filterData[key].value=$location.$$search[key].split(",");
else $scope.filterData[key].value=$location.$$search[key]}});$scope.filterByTag=function(tag){window.location.href="/#/public?tags="+tag};$scope.reset=function(){angular.forEach($scope.filterData,function(val){delete val.value})};$rootScope.$watch("user",function(user){if(user){$scope.published=Recipe.findPublic();$scope.stats=Recipe.stats()}});$rootScope.breadcrumbs=[{link:"#",title:"Home"},{link:"#",title:"Recetas Publicadas"}];$scope.addFavorites=function(recipe){User.addToFavorites(recipe,function(user){$rootScope.user.favorites=
user.favorites})};$scope.removeFavorites=function(recipe){User.removeFromFavorites(recipe,function(user){$rootScope.user.favorites=user.favorites})};$scope.newTag="";$scope.addTag=function($event){if($event.keyCode==13){if(!$scope.filterData["tags"].value)$scope.filterData["tags"].value=[];if($scope.filterData["tags"].value.indexOf($scope.newTag)==-1)$scope.filterData["tags"].value.push($scope.newTag);$scope.newTag=""}}});index.controller("RecipeListCtrl",function($scope,$rootScope,Recipe,User,$location,
$timeout,sortData,alertFactory){$scope.sort=sortData("code","-");$scope.showTags=function(recipe){if(recipe.tags&&recipe.tags.length!=0){var txt="- Tags: ["+recipe.tags[0];for(var i=1;i<recipe.tags.length;i++)txt+=", "+recipe.tags[i];return txt+"]"}else return""};$rootScope.breadcrumbs=[{link:"#",title:"Home"}];$rootScope.$watch("user",function(user){if(user){$scope.recipes=Recipe.query();$scope.stats=Recipe.stats()}});$scope.confirmationID=function(id){return"confirmation"+id.replace("(","_").replace(")",
"_")};$scope.removeRecipe=function(_id){Recipe.remove({id:_id},function(){$("#"+$scope.confirmationID(_id)).modal("hide");$timeout(function(){$scope.recipes=Recipe.query()},500)})};$scope.publish=function(recipe){recipe.$publish({isPublic:true},function(){alertFactory.create("success","La misma ya estara disponible para el resto de los usuarios!","Receta publicada con exito!")})}})})();(function(){var index=angular.module("index");index.controller("RecipeDetailHopAmountCtrl",function($scope){$scope.$watch("hop.AMOUNT",function(){$scope.amountGrs=$scope.hop.AMOUNT*1E3});$scope.amountGrs=$scope.hop.AMOUNT*1E3});index.controller("RecipeDetailCtrl",function($scope,BrewHelper,BrewCalc,Grain,Hop,HopUse,HopForm,Yeast,Style,Tag,Misc,MiscType,MiscUse,$routeParams,$rootScope,Recipe,$location,alertFactory,TagColor){$scope.BrewHelper=BrewHelper;$rootScope.breadcrumbs=[{link:"#",title:"Home"},
{link:"#",title:"Receta"}];$scope.color=TagColor;$scope.grains=Grain.query();$scope.hops=Hop.query();$scope.hopUses=HopUse.query();$scope.hopForms=HopForm.query();$scope.yeasts=Yeast.query();$scope.miscs=Misc.query();$scope.styles=Style.query();$scope.miscTypes=MiscType.query();$scope.miscUses=MiscUse.query();$scope.tags=Tag.query();$scope.round=function(value){return Math.round(value)};$scope.round1=function(value){return BrewHelper.round(value,10)};$scope.round2=function(value){return BrewHelper.round(value,
100)};$scope.removeFermentable=function(fermentable){var index=$scope.recipe["FERMENTABLES"]["FERMENTABLE"].indexOf(fermentable);$scope.recipe["FERMENTABLES"]["FERMENTABLE"].splice(index,1);$scope.changeAmount()};$scope.addFermentable=function(){$scope.recipe["FERMENTABLES"]["FERMENTABLE"].push({"NAME":"","VERSION":"1","AMOUNT":null,"TYPE":"Grain","YIELD":0,"COLOR":null,"POTENTIAL":null,"PERCENTAGE":100});$scope.changeAmount()};$scope.batchSizeBlur=function(){if((!$scope.recipe.BATCH_SIZE||$scope.recipe.BATCH_SIZE==
0)&&$scope.tempAmount)$scope.recipe.BATCH_SIZE=$scope.tempAmount};$scope.tempAmount=null;$scope.noUpdate=false;$scope.$watch("recipe.EFFICIENCY",function(newValue,oldValue){if(!oldValue||!$scope.recipe)return;if(!$scope.recipe.fixIngredients||$scope.recipe.fixIngredients=="0"){$scope.noUpdate=true;$scope.recipe.BATCH_SIZE=BrewHelper.round($scope.recipe.BATCH_SIZE*newValue/oldValue,10);$scope.changeAmount()}else $scope.changeAmount()});$scope.$watch("recipe.BATCH_SIZE",function(newValue,oldValue){if(!oldValue||
!$scope.recipe||!$scope.recipe.FERMENTABLES)return;if($scope.noUpdate){$scope.noUpdate=false;return}if(!newValue||newValue==0){$scope.tempAmount=oldValue;return}if(!$scope.recipe.fixIngredients||$scope.recipe.fixIngredients=="0"){var cohef=newValue/oldValue;var newTotalAmount=$scope.recipe.totalAmount*cohef;angular.forEach($scope.recipe.FERMENTABLES.FERMENTABLE,function(f){f.AMOUNT=BrewHelper.round(f.PERCENTAGE/100*newTotalAmount,1E3)});var newTotalHop=$scope.recipe.totalHop*cohef;angular.forEach($scope.recipe.HOPS.HOP,
function(hop){var percentage=$scope.hopPercentage(hop,$scope.recipe.totalHop);hop.AMOUNT=BrewHelper.round(percentage/100*newTotalHop,1E4)})}$scope.changeAmount()});$scope.changeAmount=function(){var amount=0;angular.forEach($scope.recipe.FERMENTABLES.FERMENTABLE,function(f){amount+=f.AMOUNT});$scope.recipe.totalAmount=amount;angular.forEach($scope.recipe.FERMENTABLES.FERMENTABLE,function(f){f.PERCENTAGE=BrewHelper.round(f.AMOUNT/$scope.recipe.totalAmount*100,100)});var colourMCU=0;angular.forEach($scope.recipe.FERMENTABLES.FERMENTABLE,
function(f){colourMCU+=f.AMOUNT/0.45359*f.COLOR/($scope.recipe.BATCH_SIZE*0.264172052637296)});$scope.recipe.CALCCOLOUR=1.4922*Math.pow(colourMCU,0.6859);var og=0;angular.forEach($scope.recipe.FERMENTABLES.FERMENTABLE,function(f){og+=BrewHelper.toLbs(f.AMOUNT)*BrewHelper.toPpg(f.POTENTIAL)*($scope.recipe.EFFICIENCY/100)/BrewHelper.toGal($scope.recipe.BATCH_SIZE)});$scope.recipe.OG=BrewHelper.toPotential(og);$scope.recipe.StrikeWater=BrewHelper.round($scope.recipe.WatertoGrainRatio*$scope.recipe.totalAmount,
10);$scope.changeHop()};$scope.hopGramsPerLiter=function(hop,batchSize){return hop.AMOUNT*1E3/batchSize};$scope.hopPercentage=function(hop,totalHop){return hop.AMOUNT/totalHop*100};function getUtilization(name,list){var utilization=1;angular.forEach(list,function(it){if(name===it.name)utilization=it.utilization});return utilization}$scope.hopIBU=function(hop){var U=BrewHelper.calculateU($scope.recipe.OG,hop.TIME);var baseIBU=BrewHelper.toOz(hop.AMOUNT)*hop.ALPHA*U*(7489/100)/BrewHelper.toGal($scope.recipe.BATCH_SIZE);
return baseIBU*getUtilization(hop.USE,$scope.hopUses)*getUtilization(hop.FORM,$scope.hopForms)};$scope.removeHop=function(hop){var index=$scope.recipe["HOPS"]["HOP"].indexOf(hop);$scope.recipe["HOPS"]["HOP"].splice(index,1);$scope.changeHop()};$scope.addHop=function(){$scope.recipe["HOPS"]["HOP"].push({"NAME":"","VERSION":"1","ALPHA":null,"AMOUNT":null,"USE":"Boil","TIME":0,"FORM":"Pellet"});$scope.changeHop()};$scope.suggests=[];$scope.changeHop=function(){var amount=0;var ibu=0;$scope.suggests=
[];angular.forEach($scope.recipe.HOPS.HOP,function(hop){amount+=hop.AMOUNT;ibu+=$scope.hopIBU(hop);$scope.suggests.push(hop.NAME)});$scope.recipe.totalHop=amount;$scope.recipe.CALCIBU=BrewHelper.round(ibu,10);$scope.changeYeast()};$scope.changeYeast=function(){var OG=BrewHelper.toPpg($scope.recipe.OG);var FG=OG*(100-$scope.recipe.YEASTS.YEAST[0].ATTENUATION)/100;$scope.recipe.FG=BrewHelper.toPotential(FG);$scope.recipe.ABV=BrewHelper.round((OG-FG)*0.131,100);var AA=(OG-FG)/OG;var RTE=0.82*FG+0.18*
OG;$scope.recipe.BV=BrewHelper.round(0.8*$scope.recipe.CALCIBU/RTE,100)};$scope.convertColor=function(srm){return BrewHelper.convertColor(srm)};$scope.calulateBUGU=function(bu,gu){return bu/BrewHelper.toPpg(gu)};$scope.changeGrain=function(fermentable){angular.forEach($scope.grains,function(grain){if(fermentable.NAME==grain.name){fermentable.POTENTIAL=grain.potential;fermentable.COLOR=grain.colour}});$scope.changeAmount()};$scope.onChangeHop=function(changed){angular.forEach($scope.hops,function(hop){if(changed.NAME==
hop.name)changed.ALPHA=hop.alpha});$scope.changeHop()};$scope.onChangeYeast=function(changed){angular.forEach($scope.yeasts,function(yeast){if(changed.NAME==yeast.name)changed.ATTENUATION=yeast.aa});$scope.changeYeast()};$scope.addMisc=function(){$scope.recipe.MISCS.MISC.push({NAME:null,VERSION:"1",TYPE:"Fining",USE:"Boil",TIME:null,AMOUNT:null})};$scope.onChangeMisc=function(changed){angular.forEach($scope.miscs,function(misc){if(changed.NAME==misc.name){changed.TYPE=misc.type;changed.USE=misc.use}})};
$scope.removeMisc=function(misc){var index=$scope.recipe.MISCS.MISC.indexOf(misc);$scope.recipe.MISCS.MISC.splice(index,1)};$scope.importEnabled=angular.isDefined(window.File)&&angular.isDefined(window.FileReader)&&angular.isDefined(window.FileList)&&angular.isDefined(window.Blob);$scope.notifications=[];$scope.bjcpLink=function(selected){var link;angular.forEach($scope.styles,function(style){if(style.link&&selected==style.name)link=style.link});return link};$scope.relatedLink=function(selected){var link;
angular.forEach($scope.styles,function(style){if(style.link&&selected==style.name)link=style.related});return link};$scope.save=function(){if(!angular.isDefined($scope.recipe.NAME)){$scope.notifications.push({type:"danger",title:"Nombre obligatorio",text:"La receta debe tener un nombre"});alertFactory.create("danger","El nombre debe ser obligatorio")}else{if(!$scope.recipe.$save)$scope.recipe=new Recipe($scope.recipe);$scope.recipe.BOIL_SIZE=$scope.BrewCalc.calculateBoilSize($scope.recipe.BATCH_SIZE,
...
[];scope.recipe.YEASTS.YEAST=[scope.recipe.YEASTS.YEAST];scope.recipe.YEASTS.YEAST[0].ATTENUATION=parseFloat(scope.recipe.YEASTS.YEAST[0].ATTENUATION);scope.recipe.GrainCalcMethod="2";scope.recipe.fermentation={view:"expand",stages:[]};scope.recipe.bottling={sugarType:"cane",bottles:[]};scope.changeYeast();scope.recipe.date=new Date;scope.$apply()}}(f);reader.readAsText(f)}}})})();(function(){var index=angular.module("login",[]);index.run(function($rootScope){$rootScope.loginSuccess=false});index.controller("LoginController",function($scope,$rootScope,User,Notification,notificationData){$scope.$on("g+login",function(event,authResult){if(authResult["access_token"]){gapi.auth.setToken(authResult);gapi.client.load("oauth2","v2",function(){var request=gapi.client.oauth2.userinfo.get();request.execute(function(obj){User.getByGoogleId({id:obj.id,name:obj.name},function(user){$rootScope.loginSuccess=
true;$rootScope.user=user;console.log(user)})})})}else if(authResult["error"]=="immediate_failed"){$rootScope.loginSuccess=true;$rootScope.$apply()}else if(authResult["error"]){$rootScope.loginSuccess=true;$scope.loginError=authResult["error"];$scope.$apply();$rootScope.$apply();console.log("There was an error: "+authResult["error"])}else{$rootScope.loginSuccess=true;$scope.loginError=JSON.stringify(authResult);$scope.$apply();$rootScope.$apply();console.log("Error inesperado")}});$scope.googleSignIn=
function(){gapi.auth.signIn(additionalParams)};notificationData.listener=function(){$scope.notificationClass="";$scope.notificationCount=0};$scope.notificationClass="";$scope.notificationCount=0;$scope.$watch("user",function(user){if(user){$scope.findNotificationsCount();setInterval($scope.findNotificationsCount,60*1E3)}});$scope.findNotificationsCount=function(){console.log("Actualizacion notificaciones");Notification.findNews(function(nots){$scope.notificationCount=nots.length;$scope.notificationClass=
nots.length!=0?"gt-notificaction-count-alert":""})}})})();(function(){var comments=angular.module("comments",[]);comments.controller("CommentController",function($scope,Recipe,$filter,$timeout){$scope.rows=1;$scope.removeComment=function(comment){var remove={comment:comment,recipe_id:$scope.recipe._id};Recipe.removeComment(remove,function(comments){$("#confirmation").modal("hide");$timeout(function(){$scope.recipe.comments=comments},500)})};$scope.addComment=function(comment){var newComment={recipe_id:$scope.recipe._id,text:comment};Recipe.addComment(newComment,
function(comments){$scope.recipe.comments=comments;$scope.comment="";$scope.rows=1})};$scope.focus=function(comment){$scope.rows=5};$scope.blur=function(comment){if(!comment||comment=="")$scope.rows=1}})})();(function(){var module=angular.module("brew-o-module.controller");module.controller("FermentationCtrl",function($scope){$scope.addFermentationStage=function(){var temp=null;if($scope.recipe.fermentation.stages.length!=0)temp=$scope.recipe.fermentation.stages[$scope.recipe.fermentation.stages.length-1].temperatureEnd;$scope.recipe.fermentation.stages.push({title:null,duration:0,durationMode:"Dias",transferring:false,losses:0,temperature:temp,temperatureEnd:null,action:null})};var today;if($scope.recipe.fermentation.estimateDate)today=
...
$scope.estimateEnd=function(day,month,year,fermentation){var date=new Date(year,month-1,day);return new Date(date.getTime()+calculateDays(fermentation)*24*60*60*1E3)};function calculateDays(fermentation){var days=0;angular.forEach(fermentation.stages,function(stage){var duration=stage.duration|0;if(duration!=0){if(stage.durationMode=="Horas")duration=duration/24;days+=duration}});return days}$scope.calculateDays=function(fermentation){var days=calculateDays(fermentation);var round=Math.round(days);
var dec=days-round;var hours=Math.round(24*dec);var result=round+" dias";if(hours!=0)result+=" y "+hours+" horas";return result};$scope.moveUp=function(stage,$index){$scope.recipe.fermentation.stages.splice($index,1);$scope.recipe.fermentation.stages.splice($index-1,0,stage)};$scope.moveDown=function(stage,$index){$scope.recipe.fermentation.stages.splice($index,1);$scope.recipe.fermentation.stages.splice($index+1,0,stage)};$scope.changeTemp=function(stage){if(!angular.isDefined(stage.temperatureEnd)||
stage.temperatureEnd==null){stage.temperatureEnd=stage.temperature;$scope.updateChart()}};$scope.styleTitle=function(onFocus){if(onFocus)return{background:"white","border-color":"#ccc"};else return{background:"#f5f5f5","border-color":"#f5f5f5",cursor:"pointer"}};$scope.updateChart=function(){var cols=[{"id":"day","label":"Dias","type":"date"},{"id":"temp","label":"Temperatura","type":"number"}];var rows=[];var day=0;var today=new Date($scope.simulatedDate_year,$scope.simulatedDate_month-1,$scope.simulatedDate_day);
angular.forEach($scope.recipe.fermentation.stages,function(stage){var duration=stage.duration|0;if(duration!=0){if(stage.durationMode=="Horas")duration=duration/24;rows.push({"c":[{"v":today},{"v":stage.temperature|0,"f":(stage.temperature|0)+"\u00ba (Inicio: "+stage.title+")"}]});day+=duration;today=new Date(today.getTime()+duration*24*60*60*1E3);rows.push({"c":[{"v":today},{"v":stage.temperatureEnd|0,"f":(stage.temperatureEnd|0)+"\u00ba (Fin: "+stage.title+") "+(stage.transferring?" trasvase":"")}]})}else rows.push({"c":[{"v":today},
{"v":stage.temperature|0,"f":(stage.temperature|0)+"\u00ba ("+stage.title+")"+(stage.transferring?" trasvase":"")}]})});$scope.chart.data.cols=cols;$scope.chart.data.rows=rows};$scope.chart={"type":"LineChart","displayed":true,"cssStyle":{height:"300px",width:"100%"},"data":{"cols":[],"rows":[]},"options":{"title":"Evolucion","isStacked":"false","fill":20,"displayExactValues":true,"vAxis":{"title":"Temperatura","gridlines":{"count":10},minValue:0},"hAxis":{"title":"Dias"}},"formatters":{}};$scope.updateChart()})})();(function(){var module=angular.module("brew-o-module.controller");module.controller("RecipeLogCtrl",function($scope,BrewCalc,BrewHelper,$timeout){var MASH_TEMP_TIME=40;var COOLING_TIME=30;$scope.discardFilter={discard:false};$scope.isFiltering=true;$scope.toogleRemovedFilter=function(){if($scope.isFiltering){$scope.discardFilter={};$scope.isFiltering=false}else{$scope.discardFilter={discard:false};$scope.isFiltering=true}};$scope.removed=false;$scope.opened=false;$scope.openDp=function(){$timeout(function(){$scope.opened=
...
discard:false});util.Arrays.remove($scope.pendingLogs,log);updatePendingTime()};$scope.discard=function(log){log.discard=true};$scope.pushAndDiscard=function(log){$scope.push(log);var last=$scope.recipe.log.logs[$scope.recipe.log.logs.length-1];$scope.discard(last)};$scope.restore=function(log){util.Arrays.remove($scope.recipe.log.logs,log);$scope.calculatePending()}})})();(function(){var notification=angular.module("notification",[]);notification.factory("notificationData",function(){return{listener:null,reset:function(){if(this.listener)this.listener()}}});notification.controller("NotificationsCtrl",function($scope,Notification,$rootScope,notificationData){notificationData.reset();$scope.updateCount=function(notifications){$scope.countUnread=0;$scope.countNew=0;angular.forEach(notifications,function(not){if(not.status=="new")$scope.countNew++;else if(not.status==
"unread")$scope.countUnread++})};$scope.$watch("user",function(){$scope.notifications=Notification.query($scope.updateCount)});$rootScope.breadcrumbs=[{link:"#",title:"Home"},{link:"#",title:"Notificaciones"}];$scope.markAsRead=function(notification){if(notification.status!="read"){notification.status="read";notification.$save();$scope.updateCount($scope.notifications)}};$scope.statusClass=function(notification){if(notification.status=="unread")return"gt-notification-unread";else if(notification.status==
"new")return"gt-notification-new";return""}})})();(function(){var abm=angular.module("abm",[]);abm.controller("AbmCtrl",function($scope,$rootScope,$routeParams,Grain,Hop,Bottle,Misc,Yeast,Style,sortData){$scope.allConfigs={Style:{data:Style,name:"Estilos",singular:"Estilo",orderBy:"name",pageSize:10,headers:[{field:"name",caption:"Nombre"},{field:"OG_Min",caption:"OG min"},{field:"OG_Max",caption:"OG max"},{field:"FG_Min",caption:"FG min"},{field:"FG_Max",caption:"FG max"},{field:"IBU_Min",caption:"IBU min"},{field:"IBU_Max",caption:"IBU max"},{field:"Colour_Min",
...
$scope.getActiveClass=function(tab){if(tab==$scope.entity)return"active";else return""}})})();(function(){var abm=angular.module("admin",[]);abm.controller("AdminCtrl",function($scope,$rootScope,$routeParams,AdminUser,AdminRecipe,Bottle,$filter,$location,AdminAction){$scope.allConfigs={Action:{data:AdminAction,name:"Acciones",singular:"Accion",canAdd:false,canRemove:false,canEdit:false,orderBy:"date",orderDir:"-",headers:[{field:"date",caption:"Fecha",format:function(value){return $filter("date")(value,"dd-MM-yyyy HH:mm")}},{field:"actionType",caption:"Accion"},{field:"data",caption:"Datos"},
{field:"_id",caption:"ID"}]},User:{data:AdminUser,name:"Usuarios",singular:"Usuario",canAdd:false,canRemove:false,canEdit:false,orderBy:"name",headers:[{field:"name",caption:"Nombre",valueTemplateUrl:"partial/admin/abm-value-user-link.html"},{field:"singInDate",caption:"Entrada",format:function(value){return $filter("date")(value,"dd-MM-yyyy HH:mm")}},{field:"lastLogin",caption:"Ultima vez",format:function(value){return $filter("date")(value,"dd-MM-yyyy HH:mm")}},{field:"_id",caption:"ID"}]},Recipe:{data:AdminRecipe,
name:"Recetas",singular:"Receta",orderBy:"NAME",canAdd:false,canRemove:false,canEdit:false,headers:[{field:"NAME",caption:"Nombre",valueTemplateUrl:"partial/admin/abm-value-link.html"},{field:"STYLE.NAME",caption:"Estilo"},{field:"date",caption:"Creacion",width:"10em",format:function(value){return $filter("date")(value,"dd-MM-yyyy")}},{field:"modificationDate",caption:"Modificacion",width:"10em",format:function(value){return $filter("date")(value,"dd-MM-yyyy")}},{field:"OG",width:"6em",caption:"OG"},
{field:"ABV",width:"6em",caption:"%Acl"},{field:"CALCIBU",width:"6em",caption:"IBUs"},{field:"isPublic",caption:"Publica",type:"checkbox"},{field:"BREWER",caption:"Cervecero"},{field:"owner.name",caption:"Due\u00f1o"}]}};$scope.getActiveClass=function(tab){if(tab==$scope.entity)return"active";else return""};$scope.context={sharedUrl:function(_id){return"http://"+$location.host()+":"+$location.port()+"/share.html#/"+_id}};$scope.entity=$routeParams.entity;$scope.config=$scope.allConfigs[$scope.entity];
$rootScope.breadcrumbs=[{link:"#",title:"Home"},{link:"#",title:$scope.config.name}]})})();(function(){var res=angular.module("resources",[]);res.factory("User",function($resource,$rootScope){var params=function(){return $rootScope.user?$rootScope.user.google_id:null};return $resource("user/:type:id",{google_id:params},{add:{method:"POST",params:{}},getByGoogleId:{method:"GET",params:{type:"google_"},isArray:false},addToFavorites:{method:"PUT",params:{type:"favorite_add"}},removeFromFavorites:{method:"PUT",params:{type:"favorite_drop"}},get:{method:"GET",params:{type:"id_"}},updateSettings:{method:"PUT",
params:{type:"settings"}}})});res.factory("Recipe",function($resource,$rootScope){var params=function(){return $rootScope.user?$rootScope.user.google_id:null};return $resource("recipe/:operation:id",{google_id:params,id:"@_id"},{findPublic:{method:"GET",params:{operation:"public"},isArray:true},findCollaborated:{method:"GET",params:{operation:"collaborated"},isArray:true},publish:{method:"POST",params:{operation:"publish_"}},findByUser:{method:"GET",params:{operation:"by_user_"},isArray:true},stats:{method:"GET",
params:{operation:"stats"}},addComment:{method:"PUT",params:{operation:"comment"},isArray:true},removeComment:{method:"PUT",params:{operation:"remove_comment"},isArray:true}})});res.factory("Notification",function($resource,$rootScope){var params=function(){return $rootScope.user?$rootScope.user.google_id:null};return $resource("notification/:_id",{google_id:params,_id:"@_id"},{findNews:{method:"GET",params:{_id:"news"},isArray:true}})});var services=["Style","Grain","Hop","Yeast","Misc","Bottle",
"Tag"];angular.forEach(services,function(s){res.factory(s,function($resource){return $resource(s[0].toLowerCase()+s.substr(1)+"/:_id",{_id:"@_id"},{})})});var admin=["User","Recipe","Action"];angular.forEach(admin,function(s){res.factory("Admin"+s,function($resource){return $resource("admin/"+s.toLowerCase()+"/:_id",{_id:"@_id"},{})})})})();(function(exports){function DiffHelper(){var ready;var result;function parentArray(parent){return{parent:parent,wrap:function(field){return parent+"["+field+"]"}}}function parentObject(parent){return{parent:parent,wrap:function(field){return parent+"."+field}}}this.compareAll=function(obj1,obj2,parent){for(var i in obj1)if(ready.indexOf(i)==-1){ready.push(i);var p;if(obj1 instanceof Array)p=parentArray("$");else p=parentObject("$");this.compare(obj1,obj2,i,parent||p)}};this.compare=function(o1,o2,
field,parent){var diff=[];if(o1[field]instanceof Date&&o2[field]instanceof Date){if(o1[field].getTime()!=o2[field].getTime())diff=[parent.wrap(field)]}else if(o1[field]instanceof Array&&o2[field]instanceof Array)diff=(new DiffHelper).diff(o1[field],o2[field],parentArray(parent.wrap(field)));else if(o1[field]instanceof Object&&o2[field]instanceof Object)diff=(new DiffHelper).diff(o1[field],o2[field],parentObject(parent.wrap(field)));else if(o1[field]!=o2[field])diff=[parent.wrap(field)];for(var i=
0;i<diff.length;i++)result.push(diff[i])};this.diff=function(obj1,obj2,parent){ready=[];result=[];this.compareAll(obj1,obj2,parent);this.compareAll(obj2,obj1,parent);return result}}exports.diff=function(obj1,obj2){return(new DiffHelper).diff(obj1,obj2)};exports.Arrays={remove:function(array,object,comparator){if(comparator){var index=-1;for(var i=0;i<array.length;i++)if(comparator(object,array[i])==0){index=i;break}if(index!==-1)array.splice(index,1);return index}else{var index=array.indexOf(object);
if(index!==-1)array.splice(index,1);return index}},filter:function(array,comparator){var result=[];for(var i=0;i<array.length;i++)if(comparator(array[i])==0)result.push(array[i]);return result}};exports.formatDate=function(date,defaultFormatter){date=new Date(date);var today=(new Date).getTime()/1E3;var dateSec=date.getTime()/1E3;var diffSec=today-dateSec;if(diffSec<0)if(diffSec>-10)return"Ahora";else if(diffSec>-60)return"En menos de un minuto";else if(diffSec>-(60*60))return"En "+Math.round(-diffSec/
60)+" minutos";else if(date.getDate()==(new Date).getDate())return"En "+Math.floor(-diffSec/60/60)+":"+exports.pad(Math.floor(-diffSec/60%60),2)+" Horas";else if(date.getDate()==(new Date).getDate()+1)return"Ma\u00f1ana "+defaultFormatter(date,"HH:mm");else return defaultFormatter(date,"dd/MM/yyyy HH:mm");else if(diffSec<10)return"Ahora";else if(diffSec<60)return"Hace menos de un minuto";else if(diffSec<60*60)return"Hace "+Math.round(diffSec/60)+" minutos";else if(date.getDate()==(new Date).getDate())return"Hoy"+
" hace "+Math.round(diffSec/60/60)+" horas";else if(date.getDate()==(new Date).getDate()-1)return"Ayer "+defaultFormatter(date,"HH:mm");else return defaultFormatter(date,"dd/MM/yyyy HH:mm")};exports.pad=function(value,zeros){value=value.toString();if(value.length>zeros)return value;else{var result=value;for(var i=0;i<zeros-value.length;i++)result="0"+result;return result}}})(typeof exports==="undefined"?this["util"]={}:exports);var data=angular.module("data",[]);data.factory("HopForm",function(){return{query:function(){return[{name:"Pellet",utilization:1},{name:"Whole Leaf",utilization:0.9},{name:"Plug",utilization:0.92}]}}});data.factory("HopUse",function(){return{query:function(){return[{name:"Boil",utilization:1},{name:"First Wort",utilization:1.1},{name:"Dry Hop",utilization:0},{name:"Aroma",utilization:0},{name:"Mash",utilization:0.2}]}}});
data.factory("MiscType",function(){return{query:function(){return["Fining","Water Agent","Spice","Other","Herb","Flavor"]}}});data.factory("MiscUse",function(){return{query:function(){return["Boil","Mash","Secondary"]}}});(function(){var env=angular.module("env",[]);env.constant("version","1.0.1-RC1");env.constant("env","1.0.1-RC1");env.constant("color","danger")})();if(typeof XML=="undefined")XML=function(){};XML.ObjTree=function(){return this};XML.ObjTree.VERSION="0.24";XML.ObjTree.prototype.xmlDecl='<?xml version="1.0" encoding="UTF-8" ?>\n';XML.ObjTree.prototype.attr_prefix="-";XML.ObjTree.prototype.overrideMimeType="text/xml";
...
fieldName);var ret=0;angular.forEach(value,function(l){if(list.indexOf(l)==-1)ret=-1});return ret}}}};gt.filter("advanced",function(){return function(rows,filterData){if(!rows)return rows;if(!filterData)return rows;else{var filters=[];angular.forEach(filterData,function(filter,field){if(filter.type!="list"&&filter.value||filter.type=="list"&&filter.value&&filter.value.length!=0){var f=fixedFilters[filter.comparator](field,filter.value,filter.ignoreCase,filter.type);filters.push(f)}});angular.forEach(filters,
function(f){rows=util.Arrays.filter(rows,f)});return rows}}});gt.directive("gtTable",function($compile,$rootScope,sortData,PAGE_SIZE){return{restrict:"EA",replace:true,scope:{config:"&",entity:"&",canRemove:"=",canEdit:"=",canAdd:"=",context:"&"},templateUrl:"template/abm.html",link:function(scope,element,attrs){},controller:function($scope){$scope.sort=sortData($scope.config().orderBy,$scope.config().orderDir||"");$scope.getActiveClass=function(tab){if(tab==$scope.entity())return"active";else return""};
$scope.addNew=function(){$scope.rows.push({_draft:true});$scope.page=$scope.getPageCount($scope.rows.length)};$scope.edit_id=null;$scope.isEditing=function(row){return row._id==$scope.edit_id};$scope.valueTemplate=function(row,header){if($scope.isEditing(row)&&!header.readonly){if(!header.type||header.type=="text"||header.type=="number")return"template/abm-input.html";if(header.type=="checkbox")return"template/abm-checkbox.html";if(header.type=="combo")return"template/abm-combo.html";else return"template/abm-input.html"}else if(header.valueTemplateUrl)return header.valueTemplateUrl;
else if(header.type=="checkbox")return"template/abm-value-checkbox.html";else return"template/abm-value.html"};$scope.edit=function(row){$scope.edit_id=row._id};$scope.copy=function(row){return angular.copy(row)};$scope.getValue=function(entity,header){var value;if(header.field.indexOf(".")!=-1){var chain=header.field.split(".");for(var i=0;i<chain.length;i++)if(entity)entity=entity[chain[i]];value=entity||"-"}else value=entity[header.field];if(header.format)return header.format(value);else return value};
$scope.remove=function(row){var clean=function(){util.Arrays.remove($scope.rows,row)};if(!row.$delete)$scope.config().data.remove(row,clean);else row.$delete(clean)};$scope.cancel=function(row,value){if(row._draft)util.Arrays.remove($scope.rows,row);else angular.forEach($scope.config().headers,function(h){value[h.field]=row[h.field]});$scope.edit_id=null};$scope.save=function(value,row){angular.forEach($scope.config().headers,function(h){row[h.field]=value[h.field]});if(row._id){if(!row.$save)$scope.config().data.save(row);
else row.$save();$scope.edit_id=null}else $scope.config().data.save(row,function(n){row._id=n._id})};$scope.page=1;$scope.rows=$scope.config().data.query();$scope.pageSize=function(){return $scope.config().pageSize||PAGE_SIZE};$scope.getPageCount=function(length){var pageSize=$scope.pageSize();return Math.ceil(length/pageSize)}}}});gt.factory("sortData",function(){return function(startField,startAsc){var data={asc:startAsc,field:startField,orderStyle:{},orderBy:function(){return this.asc+this.field},
resort:function(field){if(field==this.field)if(this.asc=="-"){this.asc="";this.orderStyle[field]="glyphicon glyphicon-chevron-up"}else{this.asc="-";this.orderStyle[field]="glyphicon glyphicon-chevron-down"}else{angular.forEach(this.orderStyle,function(style,key){data.orderStyle[key]=""});this.orderStyle[field]="glyphicon glyphicon-chevron-up";this.field=field;this.asc=""}}};if(startAsc=="-")data.orderStyle[startField]="glyphicon glyphicon-chevron-down";else data.orderStyle[startField]="glyphicon glyphicon-chevron-up";
return data}})})();
