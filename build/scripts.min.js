(function(){var index=angular.module("index",["ngResource","ngRoute","ngSanitize","ngAnimate","data","resources","helper","login","comments","googlechart","calculator","brew-o-module.controller","notification","abm","gt.abm","admin","ui.bootstrap","alerts","settings","water","env","observer"]);index.config(["$routeProvider",function($routeProvider){$routeProvider.when("/recipe",{templateUrl:"partial/recipe-list.html",controller:"RecipeListCtrl"}).when("/collaborated",{templateUrl:"partial/recipe-collaborated.html",
controller:"RecipeCollaboratedCtrl"}).when("/favorites",{templateUrl:"partial/recipe-favorite.html",controller:"RecipeFavoriteCtrl"}).when("/public",{templateUrl:"partial/recipe-public.html",controller:"RecipePublicCtrl"}).when("/home/:userId",{templateUrl:"partial/user/home.html",controller:"HomeCtrl"}).when("/recipe/edit/:recipeId",{templateUrl:"partial/recipe-detail.html",controller:"RecipeDetailCtrl"}).when("/recipe/clone/:recipeId",{templateUrl:"partial/recipe-detail.html",controller:"RecipeDetailCtrl"}).when("/recipe/new",
{templateUrl:"partial/recipe-detail.html",controller:"RecipeDetailCtrl"}).when("/settings/water",{templateUrl:"partial/user/user-settings-water.html",controller:"SettingsWaterCtrl"}).when("/settings/water/new",{templateUrl:"partial/user/settings/user-settings-water-detail.html",controller:"SettingsWaterDetailCtrl"}).when("/settings/water/:waterId",{templateUrl:"partial/user/settings/user-settings-water-detail.html",controller:"SettingsWaterDetailCtrl"}).when("/settings",{templateUrl:"partial/user/user-settings.html",
controller:"UserSettingsCtrl"}).when("/calculator",{templateUrl:"partial/calculator/calculator.html",controller:"CalculatorCtrl"}).when("/notification",{templateUrl:"partial/user/user-notification.html",controller:"NotificationsCtrl"}).when("/data/:entity",{templateUrl:"partial/data/abm.html",controller:"AbmCtrl"}).when("/admin/:entity",{templateUrl:"partial/admin/admin.html",controller:"AdminCtrl"}).otherwise({redirectTo:"/recipe"})}]);index.config(["abmProvider",function(abmProvider){abmProvider.setTemplateDir("template")}]);
index.controller("HomeCtrl",function($scope,$rootScope,User,Recipe,$routeParams){$scope.$watch("user",function(){$scope.viewUser=User.get({id:$routeParams.userId},function(){$rootScope.breadcrumbs=[{link:"#",title:"Home"},{link:"#",title:$scope.viewUser.name}]});$scope.recipes=Recipe.findByUser({id:$routeParams.userId})});$scope.addFavorites=function(recipe){User.addToFavorites(recipe,function(user){$rootScope.user.favorites=user.favorites})};$scope.removeFavorites=function(recipe){User.removeFromFavorites(recipe,
function(user){$rootScope.user.favorites=user.favorites})}});var notification=angular.module("notification",[]);notification.factory("notificationData",function(){return{listener:null,reset:function(){if(this.listener)this.listener()}}});notification.controller("NotificationsCtrl",function($scope,Notification,$rootScope,notificationData){notificationData.reset();$scope.updateCount=function(notifications){$scope.countUnread=0;$scope.countNew=0;angular.forEach(notifications,function(not){if(not.status==
"new")$scope.countNew++;else if(not.status=="unread")$scope.countUnread++})};$scope.$watch("user",function(){$scope.notifications=Notification.query($scope.updateCount)});$rootScope.breadcrumbs=[{link:"#",title:"Home"},{link:"#",title:"Notificaciones"}];$scope.markAsRead=function(notification){if(notification.status!="read"){notification.status="read";notification.$save();$scope.updateCount($scope.notifications)}};$scope.statusClass=function(notification){if(notification.status=="unread")return"gt-notification-unread";
else if(notification.status=="new")return"gt-notification-new";return""}});index.controller("MainController",function($scope,$rootScope,User){$rootScope.breadcrumbs=[];$scope.login=function(){googleSignIn()};var infos=[{text:"Si tenes 2 minutos, podes rellenar una encuesta sobre uso de Brew-o-Matic.",link:{text:"Abrir Encuesta",href:"https://docs.google.com/forms/d/1lCObRGFtB2g3S3jiwwNweNUz5hRrLuyni2zFZz40R58/viewform"},id:"closeUseSurvey"}];$scope.infos=[];$scope.closeInfo=function(index){$scope.infos.splice(index,
1)};$scope.closeForEver=function(info,index){$scope.user.settings[info.id]=true;User.updateSettings($scope.user,function(){$scope.infos.splice(index,1)})};$scope.$watch("user",function(user){if(user)for(var i=0;i<infos.length;i++)if(!user.settings[infos[i].id])$scope.infos.push(infos[i])})});var alerts=angular.module("alerts",[]);alerts.factory("alertFactory",function($rootScope){var alerts=[];return{create:function(type,text,title){alerts=[];var a={type:type,text:text,title:title};alerts.push(a);
setTimeout(function(){util.Arrays.remove(alerts,a);$rootScope.$apply()},5E3)},getAlerts:function(){return alerts}}});index.run(function($rootScope,version,$filter,$location,BrewCalc,env,color,alertFactory,BrewHelper){$rootScope.BrewCalc=BrewCalc;$rootScope.BrewHelper=BrewHelper;$rootScope.version=version;$rootScope.env=env;$rootScope.color=color;$rootScope.getAlerts=function(){return alertFactory.getAlerts()};$rootScope.encodeName=function(name){return encodeURIComponent(name)};$rootScope.sharedUrl=
...
function(){updateIFrame()};function updateIFrame(){if($scope.recipe.beer_id)$scope.iframeUrl=$sce.trustAsResourceUrl("http://www.birrasquehetomado.com.ar/html/tag.html#/beer/tag/"+$scope.recipe.beer_id)}updateIFrame()});module.controller("RecipeTabCtrl",function($scope){$scope.sortTabs=["main","mash","boil","fermentation","bottling","log","collaborators","rating"];$scope.tabs={main:{title:"Receta",template:"detail-main"},mash:{title:"Macerado",template:"mash"},boil:{title:"Hervido",template:"boil"},
fermentation:{title:"Fermentacion",template:"fermentation"},bottling:{title:"Embotellado",template:"bottling"},log:{title:"Bitacora",template:"log"},collaborators:{title:"Colaboradores",template:"collaborators"},rating:{title:"Calificaciones",template:"rating"}};$scope.selectedTab="main";$scope.changeTab=function(tab){$scope.selectedTab=tab;$scope.$parent.notifications=[]}})})();(function(){var index=angular.module("index");index.filter("limitText",function(){return function(value,limit){if(value.length>limit)return value.substring(0,limit)+"...";else return value}});index.filter("filterFavorites",function(){return function(list,favorites){var ret=[];if(list&&list.length!=0&&favorites&&favorites.length!=0)angular.forEach(list,function(recipe){if(favorites.indexOf(recipe._id)!=-1)ret.push(recipe)});return ret}});index.controller("RecipeCollaboratedCtrl",function($scope,$rootScope,
...
"_")};$scope.removeRecipe=function(_id){Recipe.remove({id:_id},function(){$("#"+$scope.confirmationID(_id)).modal("hide");$timeout(function(){$scope.recipes=Recipe.query()},500)})};$scope.publish=function(recipe){recipe.$publish({isPublic:true},function(){alertFactory.create("success","La misma ya estara disponible para el resto de los usuarios!","Receta publicada con exito!")})}})})();(function(){var index=angular.module("index");index.controller("RecipeDetailHopAmountCtrl",function($scope){$scope.$watch("hop.AMOUNT",function(){$scope.amountGrs=$scope.hop.AMOUNT*1E3});$scope.amountGrs=$scope.hop.AMOUNT*1E3});index.controller("RecipeDetailCtrl",function($scope,BrewHelper,BrewCalc,Grain,Hop,HopUse,HopForm,Yeast,Style,Tag,Misc,MiscType,MiscUse,$routeParams,$rootScope,Recipe,$location,alertFactory,TagColor,CalculatorPopup){$scope.BrewHelper=BrewHelper;$rootScope.breadcrumbs=[{link:"#",
title:"Home"},{link:"#",title:"Receta"}];$scope.color=TagColor;$scope.grains=Grain.query();$scope.hops=Hop.query();$scope.hopUses=HopUse.query();$scope.hopForms=HopForm.query();$scope.yeasts=Yeast.query();$scope.miscs=Misc.query();$scope.styles=Style.query();$scope.miscTypes=MiscType.query();$scope.miscUses=MiscUse.query();$scope.tags=Tag.query();$scope.openCalculatorOG=function(){CalculatorPopup.open({abv:true,hydrometer:false,refractometer:false},{OG:$scope.recipe.OG,FG:$scope.recipe.FG})};$scope.openCalculatorFG=
function(){CalculatorPopup.open({abv:false,hydrometer:false,refractometer:true},{OG:$scope.recipe.OG,FG:$scope.recipe.FG})};$scope.round=function(value){return Math.round(value)};$scope.round1=function(value){return BrewHelper.round(value,10)};$scope.round2=function(value){return BrewHelper.round(value,100)};$scope.removeFermentable=function(fermentable){var index=$scope.recipe["FERMENTABLES"]["FERMENTABLE"].indexOf(fermentable);$scope.recipe["FERMENTABLES"]["FERMENTABLE"].splice(index,1);$scope.changeAmount()};
...
$scope.hopForms)};$scope.removeHop=function(hop){var index=$scope.recipe["HOPS"]["HOP"].indexOf(hop);$scope.recipe["HOPS"]["HOP"].splice(index,1);$scope.changeHop()};$scope.addHop=function(){$scope.recipe["HOPS"]["HOP"].push({"NAME":"","VERSION":"1","ALPHA":null,"AMOUNT":null,"USE":"Boil","TIME":0,"FORM":"Pellet"});$scope.changeHop()};$scope.suggests=[];$scope.changeHop=function(){var amount=0;var ibu=0;$scope.suggests=[];angular.forEach($scope.recipe.HOPS.HOP,function(hop){amount+=hop.AMOUNT;ibu+=
$scope.hopIBU(hop);$scope.suggests.push(hop.NAME)});$scope.recipe.totalHop=amount;$scope.recipe.CALCIBU=BrewHelper.round(ibu,10);$scope.changeYeast()};$scope.changeYeast=function(){var OG=BrewHelper.toPpg($scope.recipe.OG);var FG=OG*(100-$scope.recipe.YEASTS.YEAST[0].ATTENUATION)/100;$scope.recipe.FG=BrewHelper.toPotential(FG);$scope.recipe.ABV=BrewHelper.round((OG-FG)*0.131,100);var AA=(OG-FG)/OG;var RTE=0.82*FG+0.18*OG;$scope.recipe.BV=BrewHelper.round(0.8*$scope.recipe.CALCIBU/RTE,100)};$scope.convertColor=
function(srm){return BrewHelper.convertColor(srm)};$scope.calulateBUGU=function(bu,gu){return bu/BrewHelper.toPpg(gu)};$scope.changeGrain=function(fermentable){angular.forEach($scope.grains,function(grain){if(fermentable.NAME==grain.name){fermentable.POTENTIAL=grain.potential;fermentable.COLOR=grain.colour}});$scope.changeAmount()};$scope.onChangeHop=function(changed){angular.forEach($scope.hops,function(hop){if(changed.NAME==hop.name)changed.ALPHA=hop.alpha});$scope.changeHop()};$scope.onChangeYeast=
function(changed){angular.forEach($scope.yeasts,function(yeast){if(changed.NAME==yeast.name)changed.ATTENUATION=yeast.aa});$scope.changeYeast()};$scope.addMisc=function(){$scope.recipe.MISCS.MISC.push({NAME:null,VERSION:"1",TYPE:"Fining",USE:"Boil",TIME:null,AMOUNT:null})};$scope.onChangeMisc=function(changed){angular.forEach($scope.miscs,function(misc){if(changed.NAME==misc.name){changed.TYPE=misc.type;changed.USE=misc.use}})};$scope.removeMisc=function(misc){var index=$scope.recipe.MISCS.MISC.indexOf(misc);
$scope.recipe.MISCS.MISC.splice(index,1)};$scope.importEnabled=angular.isDefined(window.File)&&angular.isDefined(window.FileReader)&&angular.isDefined(window.FileList)&&angular.isDefined(window.Blob);$scope.notifications=[];$scope.bjcpLink=function(selected){var link;angular.forEach($scope.styles,function(style){if(style.link&&selected==style.name)link=style.link});return link};$scope.relatedLink=function(selected){var link;angular.forEach($scope.styles,function(style){if(style.link&&selected==style.name)link=
style.related});return link};$scope.save=function(){if(!angular.isDefined($scope.recipe.NAME)){$scope.notifications.push({type:"danger",title:"Nombre obligatorio",text:"La receta debe tener un nombre"});alertFactory.create("danger","El nombre debe ser obligatorio")}else{if(!$scope.recipe.$save)$scope.recipe=new Recipe($scope.recipe);$scope.recipe.BOIL_SIZE=$scope.BrewCalc.calculateBoilSize($scope.recipe.BATCH_SIZE,$scope.recipe.TrubChillerLosses,$scope.recipe.BOIL_TIME,$scope.recipe.PercentEvap,$scope.recipe.TopUpWater);
$scope.saving=true;$scope.recipe.$save(function(saved){$scope.saving=false;$scope.notifications.push({type:"success",title:"Receta Guardada!",text:"Ya puedes acceder a esta receta desde cualquier lugar!"});alertFactory.create("success","Receta Guardada!");$location.path("/recipe/edit/"+saved._id)},function(error){if(error.status==501){alertFactory.create("warning",error.data.error,"Cuidado!");$scope.notifications.push({type:"warning",title:"Cuidado!",text:error.data.error})}else{alertFactory.create("danger",
error.data.error);$scope.notifications.push({type:"danger",title:"Error!",text:error.data.error})}})}};$rootScope.$watch("user",function(user){if(user){var now=new Date;if($routeParams.recipeId)$scope.recipe=Recipe.get({id:$routeParams.recipeId},function(){if($location.path().indexOf("/recipe/clone")==0){$scope.recipe.cloneFrom=$scope.recipe._id;$scope.recipe._id=undefined;$scope.recipe.code=undefined;$scope.recipe.date=now;$scope.recipe.modificationDate=now;$scope.recipe.starredBy=[];$scope.recipe.clonedBy=
[];$scope.recipe.isPublic=$scope.user.settings.defaultValues.isPublic;$scope.recipe.comments=[]}BrewCalc.fixEmptyValues($scope.recipe);$scope.changeYeast();$rootScope.breadcrumbs=[{link:"#",title:"Home"},{link:"#",title:"Receta - "+$scope.recipe.NAME}]});else{$scope.recipe=new Recipe({"GrainCalcMethod":"2",fixIngredients:"1",STYLE:{},date:now,modificationDate:now,totalAmount:0,totalHop:0,CALCCOLOUR:0,BATCH_SIZE:$scope.user.settings.defaultValues.BATCH_SIZE,EFFICIENCY:$scope.user.settings.defaultValues.EFFICIENCY,
OG:1,CALCIBU:0,FG:1,BOIL_TIME:$scope.user.settings.defaultValues.BOIL_TIME,BREWER:$scope.user.settings.defaultValues.BREWER,GrainAbsorbtion:$scope.user.settings.defaultValues.GrainAbsorbtion||0.9,"FERMENTABLES":{"FERMENTABLE":[]},"HOPS":{"HOP":[]},"YEASTS":{"YEAST":[{"NAME":"","VERSION":"1","ATTENUATION":75}]},MISCS:{MISC:[]},fermentation:{view:"expand",stages:[]},bottling:{sugarType:"cane",bottles:[]},WatertoGrainRatio:$scope.user.settings.defaultValues.WatertoGrainRatio,mashTemp:$scope.user.settings.defaultValues.mashTemp,
lossMashTemp:$scope.user.settings.defaultValues.lossMashTemp,GrainTemp:$scope.user.settings.defaultValues.GrainTemp,SpargeDeadSpace:$scope.user.settings.defaultValues.SpargeDeadSpace,SpargeTempDesired:$scope.user.settings.defaultValues.SpargeTempDesired,TopUpWater:0,PercentEvap:$scope.user.settings.defaultValues.PercentEvap,TrubChillerLosses:$scope.user.settings.defaultValues.TrubChillerLosses,isPublic:$scope.user.settings.defaultValues.isPublic,collaborators:[],version:[]});$scope.changeYeast()}}});
$scope.tabLink=function(tab){var base="#/recipe/edit/"+$scope.recipe._id;if(tab=="mash")return base+"/mash";return base};$scope.gravityBarValue=function(grav,max){return BrewHelper.toPpg(grav)/max*100};$scope.volumeByCarbonatationType={sugar:0,must:0,co2:0};$scope.addTag=function($event){if($event.keyCode==13){if(!$scope.recipe.tags)$scope.recipe.tags=[];if($scope.recipe.tags.indexOf($scope.recipe.newTag)==-1)$scope.recipe.tags.push($scope.recipe.newTag);$scope.recipe.newTag=""}};$scope.bottledLiters=
function(){return BrewCalc.bottledLiters($scope.volumeByCarbonatationType,$scope.recipe.bottling.bottles)};$scope.estimateLiters=function($index){return BrewCalc.estimateLiters($index,$scope.recipe.BATCH_SIZE,$scope.recipe.fermentation.stages)};$scope.handleFileSelect=function(file){var files=file.files;for(var i=0,f;f=files[i];i++){var reader=new FileReader;reader.onload=function(theFile){return function(e){var xotree=new XML.ObjTree;var xml=e.target.result;var tree=xotree.parseXML(xml);var scope=
$scope;scope.recipe=tree.RECIPES.RECIPE;scope.recipe.CALCCOLOUR=parseFloat(scope.recipe.CALCCOLOUR);scope.recipe.BATCH_SIZE=parseFloat(scope.recipe.BATCH_SIZE)||$scope.user.settings.defaultValues.BATCH_SIZE;scope.recipe.EFFICIENCY=parseFloat(scope.recipe.EFFICIENCY)||$scope.user.settings.defaultValues.EFFICIENCY;scope.recipe.OG=parseFloat(scope.recipe.OG);scope.recipe.FG=parseFloat(scope.recipe.FG);scope.recipe.CALCIBU=parseFloat(scope.recipe.CALCIBU);scope.recipe.BOIL_SIZE=parseFloat(scope.recipe.BOIL_SIZE);
scope.recipe.BOIL_TIME=parseFloat(scope.recipe.BOIL_TIME)||$scope.user.settings.defaultValues.BOIL_TIME;scope.recipe.PRIMARY_TEMP=parseFloat(scope.recipe.PRIMARY_TEMP);scope.recipe.BREWER=scope.recipe.BREWER||$scope.user.settings.defaultValues.BREWER;scope.recipe.TrubChillerLosses=parseFloat(scope.recipe.TrubChillerLosses)||$scope.user.settings.defaultValues.TrubChillerLosses;scope.recipe.mashTemp=parseFloat(scope.recipe.mashTemp)||$scope.user.settings.defaultValues.mashTemp;scope.recipe.GrainTemp=
parseFloat(scope.recipe.GrainTemp)||$scope.user.settings.defaultValues.GrainTemp;scope.recipe.SpargeTempDesired=parseFloat(scope.recipe.SpargeTempDesired)||$scope.user.settings.defaultValues.SpargeTempDesired;scope.recipe.SpargeDeadSpace=parseFloat(scope.recipe.SpargeDeadSpace)||$scope.user.settings.defaultValues.SpargeDeadSpace;scope.recipe.lossMashTemp=parseFloat(scope.recipe.lossMashTemp)||$scope.user.settings.defaultValues.lossMashTemp;scope.recipe.PercentEvap=parseFloat(scope.recipe.PercentEvap)||
...
scope.recipe.TopUpWater=0;scope.recipe.totalAmount=0;function convertFerm(ferm){ferm.AMOUNT=parseFloat(ferm.AMOUNT);ferm.COLOR=parseFloat(ferm.COLOR);ferm.POTENTIAL=parseFloat(ferm.POTENTIAL);ferm.PERCENTAGE=parseFloat(ferm.PERCENTAGE);scope.recipe.totalAmount+=ferm.AMOUNT}if(scope.recipe.FERMENTABLES.FERMENTABLE instanceof Array)angular.forEach(scope.recipe.FERMENTABLES.FERMENTABLE,convertFerm);else{convertFerm(scope.recipe.FERMENTABLES.FERMENTABLE);scope.recipe.FERMENTABLES.FERMENTABLE=[scope.recipe.FERMENTABLES.FERMENTABLE]}var times=
[0,5,10,15,20,25,30,35,40,45,50,55,60,70,80,90,100,110,120];function convertHop(hop){hop.ALPHA=parseFloat(hop.ALPHA);hop.AMOUNT=parseFloat(hop.AMOUNT);hop.TIME=parseFloat(hop.TIME);if(times.indexOf(hop.TIME)==-1){var t=times[0];var i=0;while(t<hop.TIME)t=times[++i];hop.TIME=times[i-1]}scope.recipe.totalHop+=hop.AMOUNT}scope.recipe.totalHop=0;if(scope.recipe.HOPS.HOP instanceof Array)angular.forEach(scope.recipe.HOPS.HOP,convertHop);else{convertHop(scope.recipe.HOPS.HOP);scope.recipe.HOPS.HOP=[scope.recipe.HOPS.HOP]}function convertMisc(misc){misc.TIME=
parseFloat(misc.TIME);misc.AMOUNT=parseFloat(misc.AMOUNT)}if(scope.recipe.MISCS&&scope.recipe.MISCS.MISC)if(scope.recipe.MISCS.MISC instanceof Array)angular.forEach(scope.recipe.MISCS.MISC,convertMisc);else{convertMisc(scope.recipe.MISCS.MISC);scope.recipe.MISCS.MISC=[scope.recipe.MISCS.MISC]}else scope.recipe.MISCS={MISC:[]};scope.recipe.MASH.MASH_STEPS.MASH_STEP=[];scope.recipe.YEASTS.YEAST=[scope.recipe.YEASTS.YEAST];scope.recipe.YEASTS.YEAST[0].ATTENUATION=parseFloat(scope.recipe.YEASTS.YEAST[0].ATTENUATION);
scope.recipe.GrainCalcMethod="2";scope.recipe.fermentation={view:"expand",stages:[]};scope.recipe.bottling={sugarType:"cane",bottles:[]};scope.changeYeast();scope.recipe.date=new Date;scope.disableWatchs=true;scope.$apply();scope.disableWatchs=false}}(f);reader.readAsText(f)}}})})();(function(){var index=angular.module("login",["calculator"]);index.run(function($rootScope){$rootScope.loginSuccess=false});index.controller("LoginController",function($scope,$rootScope,User,Notification,notificationData,pushListener,CalculatorPopup){$scope.openCalcPopup=function(){CalculatorPopup.open()};$scope.$on("g+login",function(event,authResult){if(authResult==null){$rootScope.loginSuccess=true;$scope.loginError="";$rootScope.$apply()}else if(authResult["access_token"]){gapi.auth.setToken(authResult);
...
discard:false});util.Arrays.remove($scope.pendingLogs,log);updatePendingTime()};$scope.discard=function(log){log.discard=true};$scope.pushAndDiscard=function(log){$scope.push(log);var last=$scope.recipe.log.logs[$scope.recipe.log.logs.length-1];$scope.discard(last)};$scope.restore=function(log){util.Arrays.remove($scope.recipe.log.logs,log);$scope.calculatePending()}})})();(function(){var notification=angular.module("notification",[]);notification.factory("notificationData",function(){return{listener:null,reset:function(){if(this.listener)this.listener()},updateListener:null,update:function(){if(this.updateListener)this.updateListener()}}});notification.controller("NotificationsCtrl",function($scope,Notification,$rootScope,notificationData,pushListener){notificationData.reset();$scope.updateCount=function(notifications){$scope.countUnread=0;$scope.countNew=0;angular.forEach(notifications,
...
$rootScope.breadcrumbs=[{link:"#",title:"Home"},{link:"#",title:$scope.config.name}]})})();(function(){var calculator=angular.module("calculator",["helper"]);calculator.controller("CalculatorCtrl",function($scope,$rootScope){$rootScope.breadcrumbs=[{link:"#",title:"Home"},{link:"#",title:"Herramientas"}];$scope.values=$scope.values||{}});calculator.directive("calculatorAbv",function(){return{restrict:"AE",templateUrl:"partial/calculator/calculator-abv.html",controller:function($scope,BrewCalc){$scope.alcohol={OG:$scope.values.OG||1.05,FG:$scope.values.FG||1.01,mode:"ABV"};function updateABV(){if($scope.alcohol.mode==
...
$scope.edit_id};$scope.valueTemplate=function(row,header){if($scope.isEditing(row)&&!header.readonly)if(!header.type||header.type=="text"||header.type=="number")return abm.templateDir+"/abm-input.html";else if(header.type=="checkbox")return abm.templateDir+"/abm-checkbox.html";else if(header.type=="combo")return abm.templateDir+"/abm-combo.html";else return abm.templateDir+"/abm-input.html";else if(header.valueTemplateUrl)return header.valueTemplateUrl;else if(header.type=="checkbox")return abm.templateDir+
"/abm-value-checkbox.html";else if(header.type=="link")return abm.templateDir+"/abm-link.html";else return abm.templateDir+"/abm-value.html"};$scope.edit=function(row){$scope.edit_id=row._id};$scope.copy=function(row){return angular.copy(row)};$scope.getValue=function(entity,header){var value;if(header.field.indexOf(".")!=-1){var chain=header.field.split(".");for(var i=0;i<chain.length;i++)if(entity)entity=entity[chain[i]];value=entity||"-"}else value=entity[header.field];if(header.format)return header.format(value);
else return value};$scope.remove=function(row){var clean=function(){util.Arrays.remove($scope.rows,row)};if(!row.$delete)$scope.config().data.remove(row,clean);else row.$delete(clean)};$scope.cancel=function(row,value){if(row._draft)util.Arrays.remove($scope.rows,row);else angular.forEach($scope.config().headers,function(h){value[h.field]=row[h.field]});$scope.edit_id=null};$scope.save=function(value,row){angular.forEach($scope.config().headers,function(h){row[h.field]=value[h.field]});if(row._id){if(!row.$save)$scope.config().data.save(row);
else row.$save();$scope.edit_id=null}else $scope.config().data.save(row,function(n){row._id=n._id})};$scope.loading=true;$scope.page=1;$scope.rows=$scope.config().data.query(function(){$scope.loading=false});$scope.pageSize=function(){return $scope.config().pageSize||PAGE_SIZE};$scope.getPageCount=function(length){var pageSize=$scope.pageSize();return Math.ceil(length/pageSize)}}}});gt.provider("abm",function(){var service={templateDir:"abm"};this.setTemplateDir=function(dir){service.templateDir=
dir};this.$get=function(){return service}});gt.factory("sortData",function(){return function(startField,startAsc,startSort){var data={sort:startSort,asc:startAsc,field:startField,orderStyle:{},orderBy:function(){if(this.sort)return this.sort;else return this.field},reverse:function(){return this.asc||this.asc=="-"},resort:function(field,sort){if(field==this.field)if(this.asc=="-"){this.asc="";this.orderStyle[field]="glyphicon glyphicon-chevron-up"}else{this.asc="-";this.orderStyle[field]="glyphicon glyphicon-chevron-down"}else{angular.forEach(this.orderStyle,
function(style,key){data.orderStyle[key]=""});this.orderStyle[field]="glyphicon glyphicon-chevron-up";this.sort=sort;this.field=field;this.asc=""}}};if(startAsc=="-")data.orderStyle[startField]="glyphicon glyphicon-chevron-down";else data.orderStyle[startField]="glyphicon glyphicon-chevron-up";return data}})})();(function(){var observer=angular.module("observer",[]);observer.run(function($location,pushListener){pushListener.socket=io.connect("http://"+$location.host())});observer.factory("pushListener",function(){return{on:function(id,callback){this.socket.on(id,callback)},off:function(id,fn){if(fn)this.socket.removeListener(id,fn);else this.socket.removeAllListeners(id)}}})})();
